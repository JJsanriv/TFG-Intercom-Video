...
A continuación, se muestra los parametros inicializados del módulo, así como, los distintos dispositivos que esta usando el sistema.
\begin{lstlisting}[language=bash,basicstyle=\ttfamily\scriptsize]
InterCom parameters:
...
Empezaremos por probar el módulo con un ancho de banda muy bajo, 1 Mbps. 
...
\textbf{Análisis de \textit{Minimal\_Video\_FPS} con 50 Mbps:}
\vspace{\baselineskip}
...
\textbf{Pruebas con pérdida de paquetes del 5\%:}
\vspace{\baselineskip}
...
\begin{table}[H] 
    \centering
    \renewcommand{\arraystretch}{1.25} % Ajusta el espaciado vertical de las filas
    \begin{tabularx}{\textwidth}{|>{\itshape\arraybackslash\scriptsize}p{3.2cm}|*{9}{>{\raggedright\arraybackslash\tiny}X|}} % Ancho primera col, datos en \tiny
    \hline
...
% Restaurar el valor original de \tabcolsep si es necesario para el resto del documento
