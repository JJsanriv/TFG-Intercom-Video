...
class Minimal_Video(minimal.Minimal):  # Hereda de minimal.Minimal
    def __init__(self):
        minimal.args = args  # Se asigna el objeto args global del parser
        self.is_local_mode = (args.destination_address == "localhost")
        super().__init__()  # Inicializa la parte de audio
        ...
        try:
            ...
            if not self.cap.isOpened():
                raise IOError("No se pudo abrir cámara.")
            if args.width > 0:
    ...
    def capture_video_loop(self):
        """Captura frames y los prepara para envío/display local."""
        ...
        while self.running:
            ...
            try:
                ...
                for start in range(0, total_len, self.effective_video_payload_size):
                    ...
                    try:
                        # Se envia el paquete sin llamar a _on_send
    ...
    def receive_video(self):
        ...
        try:
            ...
            # Se elimina la llamada a _on_receive; se procesa el paquete directamente.
    ...
    def clean_old_frames(self):
        """Limpia frames incompletos o inválidos."""
        if self.is_local_mode or not self.recv_frames:
            return
        with self.recv_frames_lock:
            now = time.time()
            timeout = 1.5
    ...
    def run(self):
        ...
        for t in threads:
            t.start()
        try:
            # Llamar al run() de minimal.Minimal (audio)
            super().run()
        except KeyboardInterrupt:
            print("Interrupción por teclado detectada.")
        finally:
            print("Deteniendo aplicación (video)...")
            self.running = False
...
# Versión Verbose combinando video y estadísticas (heredadas de minimal.Minimal__verbose)
...
if __name__ == "__main__":
    ...
    # Se activa el modo verbose si se especifica --show_stats, --show_samples o --show_spectrum
    ...
    try:
        ...
        if hasattr(intercom_app, 'print_final_averages') and callable(intercom_app.print_final_averages):
            # Espera breve para asegurar el procesamiento final
            time.sleep(0.2)
